...
let
  ...
  tensorflowCudaEnv =
    if tensorflowCudaPkg != null then pkgs.python3.withPackages (_: [ tensorflowCudaPkg ]) else null;
  primaryPythonPackage = if tensorflowCudaEnv != null then tensorflowCudaEnv else pkgs.python3;
  hashcatPkg = pkgs.hashcat or null;
  hybridPackages = lib.optionals isHybridGpu (
    lib.optional (ollamaVulkan != null) ollamaVulkan ++ lib.optional (hashcatPkg != null) hashcatPkg
  );
